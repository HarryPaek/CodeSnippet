...
Sub Exceute(workingSheet As Worksheet, iOSheetFileFullPath As String, maxNumberOfRowsPerSheet As Long)
    ...
    Dim totalAreaSheet As Worksheet                               ' DATA_SHEET_TOTAL_AREAS
    ...
    Set ioSheetColumnData = ioSheetDataSheet.Range(Cells(clsCommon.ROW_INDEX_TOTAL_IOLIST_DATA_START, _
    ...
    For Each workingSheetCell In workingSheetFirstRow.Cells                                                                '통합파일 첫 행 각 셀을 순환
        For Each ioSheetCell In ioSheetFirstRow.Cells                                                                      '데이터 파일 첫 행 각 셀을 순환
            If workingSheetCell = ioSheetCell Then                                                                         '두 데이터가 일치하면
                ...
                If ioSheetDataSheet.Cells(clsCommon.ROW_INDEX_TOTAL_IOLIST_DATA_START, ioSheetCell.Column) <> "" Then      'IO Sheet 첫 행에 데이터가 있는 경우에만
   ...
   '================================================================================================================================================================
    ...
    For rowIndex = 0 To dataRowCount - 1
        ...
        For Each columnMapping In columnMappingList
            cellValues.add Item:=ioSheetDataSheet.Cells(clsCommon.ROW_INDEX_TOTAL_IOLIST_DATA_START + rowIndex, columnMapping.SourceColumn).value, key:=columnMapping.TargetColumnText
        Next columnMapping
   ...
   ' 수집된 정렬 시트의 각 행을 처리
   '================================================================================================================================================================
    Set totalAreaSheet = Worksheets(Constants.DATA_SHEET_TOTAL_AREAS)
    Set totalProcessorFactory = New ClsTotalProcessorFactory
    
    Set workingSheet = totalProcessorFactory.Process(workingSheet, totalAreaSheet, cellValuesList, columnMappingList, maxNumberOfRowsPerSheet)

    clsCommon.SetBasicColumnsHeaderText workingSheet                                                             ' 작업 시트, 기본 Cell Header Text 값 복원

    Set totalProcessorFactory = Nothing                                                                          ' 개체변수(들) 초기화(메모리 비우기)
    Set totalAreaSheet = Nothing                                                                                 ' 개체변수(들) 초기화(메모리 비우기)
